...
;(() => {
  ...
  function cssPath(el) {
    ...
    for (; el && el !== document.body; el = el.parentElement) {
      ...
      if (el.classList.length)
        s += `.${[...el.classList].map(CSS.escape).join(".")}`
      const sibs = [...el.parentNode.children].filter(
  ...
  function pick({ multi = false } = {}) {
    return new Promise((resolve) => {
      ...
      const legend = multi
        ? "ctrl/⌘+click = add · Enter = finish · Esc = cancel"
        : "click an element · Esc = cancel"
      ...
      const finish = (value) => {
        for (const type in handlers)
          document.removeEventListener(type, handlers[type], true)
      ...
      const handlers = {
        ...
        keydown(e) {
          ...
          else if (e.key === "Enter" && multi && chosen.length)
            (e.preventDefault(), finish(chosen))
        },
      }
      for (const type in handlers)
  ...
  document.addEventListener("keydown", (e) => {
    if (e.altKey && e.key.toLowerCase() === "p")
      (e.preventDefault(), ACTIONS[0].run())
  })
