...
Pluggable inputs and outputs for `df:triage`. Nothing here is required — every path degrades to "ask the user to paste the text" or "save to /tmp".
...
| ----------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **Azure DevOps**  | `az` on PATH + `azure-devops` extension; URL has `_workitems/edit/<id>` | Invoke the `az-cli` skill, then `az boards work-item show --id <id> --expand all -o json` |
...
| `--print`                                                                                              | stdout                                                                                           |
| `--out <path>`                                                                                         | that path                                                                                        |
| An artifact-protocol skill is available                                                                | invoke it and let it decide the path                                                             |
