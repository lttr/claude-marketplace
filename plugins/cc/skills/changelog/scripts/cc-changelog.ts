...
async function fetchReleaseDates(): Promise<Map<string, string>> {
  ...
  const output =
    await $`gh api repos/anthropics/claude-code/releases --paginate --jq ${jq}`
      .noThrow()
      .text()
    ...
    for await (const entry of Deno.readDir(SKILLS_DIR)) {
      ...
      try {
        const content = await Deno.readTextFile(
          `${SKILLS_DIR}/${entry.name}/SKILL.md`,
        )
...
async function fetchChangelog(): Promise<string> {
  const content =
    await $`gh api repos/anthropics/claude-code/contents/CHANGELOG.md --jq '.content'`.text()
  // Decode base64
...
async function fetchTags(): Promise<string[]> {
  const output =
    await $`gh api repos/marckrenn/claude-code-changelog/tags --paginate --jq '.[].name'`
      .noThrow()
      .text()
...
async function main() {
  ...
  const effectiveLastVersion =
    lookbackDays !== undefined ? null : state.lastVersion
  ...
  const versions = filterVersionsInWindow(
    allVersions,
  ...
  const oldestInWindow =
    versions.length > 0
      ...
      : effectiveLastVersion
  const systemPromptDiff = await fetchSystemPromptDiff(
    oldestInWindow,
    currentVersion,
  )
