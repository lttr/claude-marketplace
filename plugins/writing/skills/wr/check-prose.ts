...
export function analyze(source: string): Finding[] {
  ...
  prose.forEach((text, i) => {
    ...
    if (/;/.test(text)) {
      findings.push({
        ...
        line,
        rule: "semicolon",
        message: "semicolon splice: consider two sentences",
      })
    }
  ...
  for (const p of paragraphs) {
    ...
    if (
      !p.isListItem &&
      (sentences.length > MAX_PARAGRAPH_SENTENCES ||
        pWords > MAX_PARAGRAPH_WORDS)
    ) {
      findings.push({
        level: "WARN",
        line: p.line,
  ...
  if (proseLengths.length > 0) {
    ...
    if (a.length >= MIN_SENTENCES_FOR_SPREAD && spread < MIN_SPREAD_RATIO) {
      findings.push({
        ...
        rule: "flat-rhythm",
        message:
          `sentence lengths are uniform (p75/p25 = ${spread.toFixed(2)}, want >= ${MIN_SPREAD_RATIO}): ` +
...
function sharedGram(a: string, b: string): string | null {
  if (
    wordCount(a) < MIN_ECHO_SENTENCE_WORDS ||
    wordCount(b) < MIN_ECHO_SENTENCE_WORDS
  )
    return null
  const grams = (s: string): Set<string> => {
    const w = s.toLowerCase().match(/[a-z0-9'\u2019-]+/g) ?? []
    const out = new Set<string>()
    for (let i = 0; i + ECHO_GRAM_WORDS <= w.length; i++)
      out.add(w.slice(i, i + ECHO_GRAM_WORDS).join(" "))
    return out
  }
  const bg = grams(b)
...
async function main() {
  ...
  if (file === "-") {
    if (stdin.isTTY) {
      console.error(
        "usage: check-prose.ts <file.md> [--json], or pipe text on stdin",
      )
      process.exit(64)
    }
  ...
  if (json) {
    ...
    for (const f of findings) {
      console.log(
        `${f.level.padEnd(5)} ${f.line ? `L${f.line}` : "-"} [${f.rule}] ${f.message}`,
      )
    }
