...
| ----------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Azure DevOps**  | `az` on PATH + `azure-devops` extension; URL has `_workitems/edit/<id>` | `az boards work-item show --id <id> --expand all -o json`. The `ticket` skill (ships with the `dev-azdo` plugin) covers projections and linked PRs via `ticket show <id>` — invoke it if installed; otherwise use the command as written. |
