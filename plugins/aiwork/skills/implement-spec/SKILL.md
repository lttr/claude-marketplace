...
Throughout: keep `implementation-notes.md` in the task folder (an `aiwork-protocol` artifact) as a log for the maintainer — deliberate decisions and important notes only, recorded as they happen, not at the end: design decisions where the spec was ambiguous, intentional deviations from the spec and why, tradeoffs considered, open questions, a stopped chain and why.
