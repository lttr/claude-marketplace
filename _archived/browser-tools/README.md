...
Built before Claude Code had CLI-based browser skills. The goal was low-token browser interaction inside Claude Code: a handful of focused shell scripts instead of 20+ MCP tools, so the model could drive a browser without blowing the context window.
